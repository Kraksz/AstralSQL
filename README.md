...
Live network checks are reproducible with `cargo test --locked --no-default-features --test network_integration --test network_acceptance -- --ignored --nocapture`. They require an isolated database named `astral_test` and `ASTRAL_TEST_DRIVER`, `ASTRAL_TEST_HOST`, `ASTRAL_TEST_PORT`, `ASTRAL_TEST_USER`, and `ASTRAL_TEST_PASSWORD` environment variables. Drivers are `mysql`, `mariadb`, and `postgres`. Set `ASTRAL_TEST_SSL_MODE` to select TLS behavior (default: `disable`, for isolated fixtures only). Set `ASTRAL_TEST_UNTRUSTED_TLS=1` only when testing a deliberately untrusted certificate; this additionally asserts certificate rejection. Supply credentials locally, never in issues or chat. The tests create and drop uniquely named fixture tables, exercise writes, and deliberately reject invalid authentication and SQL. Never point these checks at a user database. A separate optional OS-vault check is available with `cargo test --no-default-features --test keychain_integration -- --ignored`; it removes its temporary credential after reading it.

On Windows, `scripts/test-databases-windows.ps1 -BinaryRoot <portable-binaries-directory>` provisions disposable loopback servers with generated credentials and test certificates. The directory must contain `mysql-8.4.11-winx64/bin`, `mariadb-11.4.10-winx64/bin`, and `pgsql/bin` (PostgreSQL 17.11). Git for Windows supplies OpenSSL at the path declared in the script. Download these binaries from their vendors before running the harness; it does not install services or expose a public listener. Run it without another Cargo build using the same target directory to avoid Windows executable locks. It tests both `localhost` and `127.0.0.1`, with TLS disabled and required, and prints a temporary JSON result-manifest path. Fixture files and logs remain in that temporary directory for inspection; servers are stopped after each engine.

See [the final database validation report](docs/final-database-validation.md) for verified results and the outstanding remote-server checks. Vitest is restricted to the current `tests/` directory so archived source packages cannot inflate the test totals.
